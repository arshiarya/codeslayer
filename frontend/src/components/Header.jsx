...
const Header = ({ onSignOut }) => {
  ...
  return (
    <header className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-50">
      ...
      <nav
        id="mobile-nav-menu"
        className={`md:hidden absolute top-14 left-0 w-full bg-white shadow-xl transition-transform duration-300 ease-in-out z-40 ${
        ...
        <div className="flex flex-col p-4 space-y-2">
            {navItems.map((item) => (
                <Link
                    ...
                    className={`block px-3 py-2 text-base rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-colors ${
                        location.pathname === item.href
