...
const ChatPage = ({ onSignOut }) => {
  ...
  const resetChat = async () => {
    if (window.confirm('Start a new conversation? This will clear your chat history.')) {
      try {
        // Attempt to reset the chat session on the backend
        ...
        // Still clear the local messages even if backend reset fails
        setMessages([
          {
            id: 1,
            text: "Hey, I'm here for you whenever you need. How are you feeling right now?",
  ...
  return (
    <div className="flex flex-col h-screen font-sans bg-gradient-to-b from-[#B5D8EB] to-[#F4F8FB]">
      {/* Header */}
      <Header onSignOut={onSignOut} />

      {/* Connection Error Banner */}
      {connectionError && (
        <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-3 mx-4 mt-2 rounded-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
              </svg>
              <p className="text-sm font-medium">Cannot connect to backend server. Check the console for details.</p>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="space-y-2">
              {moreOptions.map((option, index) => (
                <button
                  ...
                  onMouseEnter={(e) => {
                    if (!isLoading && sessionId) {
                      e.currentTarget.style.backgroundColor = 'rgb(40 138 143)';
                    }
                  }}
                  onMouseLeave={(e) => {
                    if (!isLoading && sessionId) {
                      e.currentTarget.style.backgroundColor = 'rgb(40 138 143 / 92%)';
                    }
                  }}
                >
